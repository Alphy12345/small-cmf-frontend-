...
const InspectionPlan = () => {
  ...
  const [viewMode, setViewMode] = useState('plan'); // 'plan' or 'measure'
  ...
  // Load changed rows from localStorage on component mount
  useEffect(() => {
    const savedChangedRows = localStorage.getItem(`changedRows_${partId}`);
    if (savedChangedRows) {
      try {
        const parsedRows = JSON.parse(savedChangedRows);
        setChangedRows(new Set(parsedRows));
        // Set hasUnsavedChanges to true if there are any saved changed rows
        if (parsedRows.length > 0) {
          setHasUnsavedChanges(true);
        }
      } catch (error) {
        console.error('Error parsing saved changed rows:', error);
      }
    }
  }, [partId]);

  // Save changedRows to localStorage whenever it changes
  useEffect(() => {
    if (changedRows.size > 0) {
      localStorage.setItem(`changedRows_${partId}`, JSON.stringify([...changedRows]));
    } else {
      localStorage.removeItem(`changedRows_${partId}`);
    }
  }, [changedRows, partId]);
...
if (partId) {
  ...
  const handleCellEditSave = () => {
    ...
    setChangedRows(prev => new Set([...prev, editingCell.rowId]));
    setHasUnsavedChanges(true);
...
const pdf = new jsPDF('p', 'mm', 'a4');
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      // Only clear unsaved changes flag, keep changedRows to maintain red highlight
                ...
                {bomData.length > 0 ? (
                  bomData.map((item, rowIndex) => {
                      ...
                      return (
                        <span
                          ...
                          style={{ 
                            ...
                            display: 'block',
                            padding: '4px 8px',
                            background: changedRows.has(item.id) ? '#fef3c7' : 'transparent',
                            borderRadius: '4px',
                    ...
                    const rowHighlightClass = (useBackendStatus && goNoGo === 'GO') || tolStatus === 'in' ? 'row-highlight-go'
                      ...
                      : changedRows.has(item.id) ? 'row-highlight-edited' : '';
                    
                    // Debug: Log when row should be highlighted
                    if (changedRows.has(item.id)) {
                      console.log('Row', item.id, 'should be highlighted with red');
                    }
